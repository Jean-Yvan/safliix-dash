      ...
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-4 bg-neutral rounded-2xl border border-base-300 p-4 space-y-4">
          ...
          <div className="flex items-center gap-2">
            <span
              className={`badge ${
                user.status === "actif" ? "badge-success" : "badge-error"
              }`}
            >
              {user.status}
            </span>
          ...
          <div className="flex items-center gap-2 pt-2">
            ...
            <button className="btn btn-ghost btn-sm text-white border-base-300">
              Suspendre
            </button>
          </div>
        </div>
        ...
        <div className="col-span-8 grid grid-cols-2 gap-3">
          <div className="bg-neutral rounded-2xl border border-base-300 p-4 space-y-2">
            ...
            <p className="text-white/60 text-sm">
              Paiement : {user.subscription.paymentMethod}
            </p>

            {user.subscription.invoicesDue > 0 && (
              <p className="text-warning text-sm">
                Factures impayées : {user.subscription.invoicesDue}
              </p>
            )}

            <div className="flex gap-2 pt-2">
              <button className="btn btn-ghost btn-xs text-primary border border-primary/50 rounded-full">
                Voir factures
              </button>
              <button className="btn btn-ghost btn-xs text-white border-base-300 rounded-full">
                Changer de plan
              </button>
            </div>
          </div>
          ...
          <div className="bg-neutral rounded-2xl border border-base-300 p-4 space-y-3 col-span-2">
            ...
            <ul className="space-y-2 text-sm text-white/80">
              {user.activity.map((act, idx) => (
                <li
                  key={idx}
                  className="flex items-center justify-between bg-base-200/30 border border-base-300 rounded-lg px-3 py-2"
                >
          ...
          <div className="bg-neutral rounded-2xl border border-base-300 p-4 space-y-2">
            ...
            <div className="space-y-2 text-sm text-white/80">
              {user.issues.map((issue, idx) => (
                <div
                  key={idx}
                  className="border border-base-300 rounded-lg px-3 py-2 flex items-center justify-between"
                >
                  <div>
